""" Module for the nodes of the DAG """
...
def get_data(def_fecha=pd.Timestamp.now().date()):
...
def forecast_transacciones(data: pd.DataFrame):
    """ Forecast the transaction volume for the next 7 days
    ...
    # Plot line in mean_last_7_days
    mean_last_7_days = data["y"].head(7).mean()
    ax.axhline(mean_last_7_days, color="r", linestyle="--", label="Mean last 7 days")

    # Plot line in mean_next_7_days
    mean_next_7_days = forecast[forecast["ds"] > pd.to_datetime(data["ds"].max())]["yhat"].head(7).mean()
    ax.axhline(mean_next_7_days, color="g", linestyle="--", label="Mean next 7 days")

    # Plot vertical line in last day of data
    ax.axvline(pd.to_datetime(data["ds"].max()), color="b", linestyle="--", label="Last day of data")

    plt.legend()

    plt.xticks(rotation=25)

    # Add mean_last_7_days and mean_next_7_days numbers to the y axis ticks
    yticks = ax.get_yticks()
    yticks = list(yticks) + [mean_last_7_days, mean_next_7_days]
...
def calc_variation(data, forecast):
    ...
    data_last_week = data["y"].head(7).mean()
    ...
    data_last_day = data["y"].head(1).values[0]
...
def update_tna(pe):
    ...
    if res is None:
        res = {"valor": 0.5, "fecha": pd.Timestamp.now().date()}
...
def predict_transaction_volume_update_tna(def_fecha=pd.Timestamp.now().date()):
    ...
    var, pe = calc_variation(data, forecast)
